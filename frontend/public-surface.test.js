...
test("history catalog and story detail have stable public routes", () => {
  ...
  assert.match(engineJs, /homeReadButton\.addEventListener\("click", \(\) => \{[\s\S]*navigateTo\("\/history"\)/);
  assert.match(engineJs, /card\.addEventListener\("click", \(\) => navigateTo\(storyRoute\(story\)\)\)/);
  assert.match(engineJs, /storyDetailBack\.addEventListener\("click", \(\) => navigateTo\("\/history"\)\)/);
});
